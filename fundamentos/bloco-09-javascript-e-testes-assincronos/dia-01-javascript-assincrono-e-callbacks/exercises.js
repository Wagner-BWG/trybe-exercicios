...
console.log('Lines are executed in order A, B, C');
console.log('=================================================================================');

// 2 - Agora, para o mesmo código, qual a ordem de finalização de execução das linhas comentadas abaixo?

console.log(planetDistanceFromSun(mars)); // A
setTimeout(() => console.log(planetDistanceFromSun(venus)), 3000); // B
setTimeout(() => console.log(planetDistanceFromSun(jupiter)), 2000); // C

setTimeout(() => console.log('Lines are executed in order A, C, B'), 3500);
...
// definição da função sendMarsTemperature...
const sendMarsTemperature3 = (callback, errorHandler) => {
    temperature = getMarsTemperature();
    setTimeout(() => {
        const errorChance = 0.6;
        const hit = Math.random();
        if (hit > errorChance) {
            callback(temperature);
        } else {
            errorHandler('No response from Curiosity');
        }
    },
    messageDelay());
}
